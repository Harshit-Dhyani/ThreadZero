...
function SearchDialog() {
  ...
  return <dialog ref={dialog} onCancel={(event) => { event.preventDefault(); closeWithFocus(); }} className="w-[min(720px,calc(100%-2rem))] rounded-special border-0 bg-white p-0 text-ink shadow-2xl"><div className="flex items-center gap-3 border-b border-line p-4"><Search className="size-5 text-muted" /><input ref={input} type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={language === "hi" ? "पेज, कार्य या प्रश्न खोजें" : "Search pages, tasks, or questions"} className="min-h-11 min-w-0 flex-1 border-0 bg-transparent px-2 outline-none" /><button aria-label="Close search" className="grid size-11 place-items-center rounded-full hover:bg-civic-50" onClick={closeWithFocus}><X className="size-5" /></button></div><div className="max-h-[70dvh] overflow-y-auto p-4" aria-live="polite">{!query ? <p className="p-4 text-sm text-muted">{language === "hi" ? "“पैसा गया”, “साक्ष्य” या “शिकायत ट्रैक” आजमाएँ।" : "Try “lost money”, “evidence”, or “track complaint”."}</p> : !results.length ? <p className="p-4 text-sm text-muted">{language === "hi" ? "कोई मिलान नहीं मिला।" : "No matching route found."}</p> : <>{results.map((result, index) => <button key={result.id} className={`flex min-h-16 w-full items-center justify-between gap-4 border-b border-line p-4 text-left last:border-b-0 ${index === 0 ? "bg-civic-50" : "hover:bg-canvas"}`} onClick={() => go(result.route)}><span><span className="block text-[11px] font-semibold uppercase tracking-[0.1em] text-civic-700">{index === 0 ? (language === "hi" ? "सर्वोत्तम उत्तर" : "Best answer") : result.workspace}</span><strong className="mt-1 block text-sm">{result[language].title}</strong><span className="mt-1 block text-xs leading-5 text-muted">{result[language].body}</span></span><ArrowRight className="size-4 shrink-0" /></button>)}</>}</div></dialog>;
}
