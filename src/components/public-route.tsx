...
function LearningHub({ route }: { route: LocalizedRoute }) {
  const { language } = usePortal();
  const byRoute = (id: string) => route.items.find((item) => item.route === id);
  const shortcuts = [byRoute("safety"), byRoute("advisories"), byRoute("guides")].filter(Boolean) as LocalizedRoute["items"];
  const taxonomy = [byRoute("safety"), byRoute("awareness"), byRoute("advisories"), byRoute("training")].filter(Boolean) as LocalizedRoute["items"];
  return <><section className="rounded-panel border border-line bg-white p-5 sm:p-7"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-civic-700">{language === "hi" ? "अपनी स्थिति से शुरू करें" : "Start with your situation"}</p><div className="mt-4 divide-y divide-line">{shortcuts.map((item) => <Link key={item.title} href={`/${item.route}`} className="flex min-h-20 items-center gap-4 py-4"><span className="grid size-11 shrink-0 place-items-center rounded-panel bg-civic-50 text-civic-700"><ArrowRight className="size-5" /></span><span className="flex-1"><strong className="block">{item.title}</strong><span className="mt-1 block text-sm leading-6 text-muted">{item.body}</span></span></Link>)}</div></section><section className="mt-8"><h2 className="text-2xl font-semibold">{language === "hi" ? "सभी संसाधन देखें" : "Browse all resources"}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{taxonomy.map((item) => <Link key={item.title} href={`/${item.route}`} className="min-h-28 rounded-panel border border-line bg-white p-5"><strong>{item.title}</strong><span className="mt-2 block text-sm leading-6 text-muted">{item.body}</span></Link>)}</div><div className="mt-4 flex flex-wrap gap-4 text-sm font-semibold text-civic-700">{route.items.filter((item) => !taxonomy.includes(item) && !shortcuts.includes(item)).map((item) => <Link key={item.title} href={`/${item.route}`} className="inline-flex min-h-11 min-w-11 items-center justify-center underline underline-offset-4">{item.title}</Link>)}</div></section></>;
}
