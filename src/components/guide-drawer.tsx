...
export function GuideDrawer() {
  ...
  useEffect(() => {
    ...
    if (!dialog) return;
    if (guideOpen) {
      setChoice(CHOICES.includes(guidePreset as typeof CHOICES[number]) ? guidePreset : "");
      setNarrative("");
      if (!dialog.open) {
  ...
  return <dialog ref={dialogRef} onCancel={(event) => { event.preventDefault(); close(); }} onClose={() => guideOpen && closeGuide()} className="m-0 ml-auto h-dvh max-h-none w-full max-w-[720px] overflow-y-auto border-0 bg-white p-0 text-ink shadow-2xl">
    <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-line bg-white px-5 py-5 sm:px-7"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-civic-700">{language === "hi" ? "डेमो मार्गदर्शन" : "Demo guidance"}</p><h2 className="mt-2 text-2xl font-semibold">{language === "hi" ? "ThreadZero मार्गदर्शिका" : "ThreadZero Guide"}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted">{language === "hi" ? "एक कार्य चुनें या उदाहरण घटना बताएँ। कोई AI या नेटवर्क अनुरोध नहीं होता।" : "Choose a task or describe an example incident. No AI or network request is used."}</p></div><button ref={first} aria-label={language === "hi" ? "मार्गदर्शिका बंद करें" : "Close Guide"} className="grid size-11 shrink-0 place-items-center rounded-full border border-line hover:bg-civic-50" onClick={close}><X className="size-5" /></button></div>
