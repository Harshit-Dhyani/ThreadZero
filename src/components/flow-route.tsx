...
import { canEnterRoute, createReportPreparationPack, evidenceForReportKind, formatDateTime, formatMoney, getStepIndex, markRouteComplete, prepareSimulation, validateDetails, validateEvidence } from "@/domains/report";
...
function resetAfterReportKindChange(draft: ReportState) {
  draft.completed = [];
...
export function FlowRoute({ routeId }: { routeId: string }) {
  ...
  return <div className="mx-auto max-w-content px-5 py-8 md:px-8 md:py-10">
    <nav aria-label="Breadcrumb" className="flex items-center text-xs text-civic-700"><Link className="inline-flex min-h-11 min-w-11 items-center" href="/">{t(language, "Home", "होम")}</Link><span className="px-2">/</span><Link className="inline-flex min-h-11 min-w-11 items-center" href="/incident">{t(language, "Report", "रिपोर्ट")}</Link><span className="px-2">/</span><span>{c.flow.routes[routeId as keyof typeof c.flow.routes]}</span></nav>
...
function IncidentStep({ complete, fail }: { complete: () => void; fail: (errors: Record<string, string>) => void }) {
  ...
  const chooseKind = (kind: Exclude<ReportKind, "unselected">) => mutateReport((draft) => {
    if (draft.reportKind === kind) return;
    resetAfterReportKindChange(draft);
    draft.reportKind = kind;
    draft.reportingMode = "standard";
    draft.womenChildCategory = "";
    draft.incidentChoice = kind === "unsure" ? "unsure" : "";
    if (kind === "financial") {
      draft.incident.type = "financial";
      draft.incident.narrative = "A demo investment contact promised returns, asked for a UPI payment, and stopped responding after payment.";
      draft.incident.contactChannel = "WhatsApp";
    } else if (kind === "women-child") {
      draft.incident.type = "women-child";
      draft.incident.narrative = "A demo social-media account shared harmful content and messages. This example contains no real names, contact details, or uploaded content.";
      draft.incident.contactChannel = "Social media";
    } else if (kind === "other") {
      draft.incident.type = "other-cybercrime";
      draft.incident.narrative = "A demo social-media account was accessed without permission and unfamiliar activity appeared shortly afterwards.";
      draft.incident.contactChannel = "Social media";
    } else {
      draft.incident.type = "unsure";
      draft.incident.narrative = "A demo incident happened online, but the closest cybercrime category is not yet clear. The available facts can still be organised first.";
      draft.incident.contactChannel = "Other";
    }
  });

  const chooseFinancial = (value: string) => mutateReport((draft) => { draft.incidentChoice = value; draft.incident.type = `financial-${value}`; });
  ...
  const chooseOther = (value: string) => mutateReport((draft) => { draft.incidentChoice = value; draft.incident.type = `other-${value}`; });
  ...
  return <>
    <Heading section={{ eyebrow: s.eyebrow, title: s.title, intro: t(language, "Start with your situation. The report adapts after this choice, so you do not need to understand the portal’s internal categories first.", "अपनी स्थिति से शुरू करें। इस चुनाव के बाद रिपोर्ट बदलती है, इसलिए पहले पोर्टल की आंतरिक श्रेणियाँ समझना जरूरी नहीं है।") }} />
    ...
    <button className="mt-5 min-h-11 text-sm font-semibold text-civic-700 underline" onClick={() => openGuide("report")}>{t(language, "Need help choosing? Open the Guide", "चुनने में मदद चाहिए? मार्गदर्शिका खोलें")}</button><br />
...
function DetailsStep({ complete, fail }: { complete: () => void; fail: (errors: Record<string, string>) => void }) {
  ...
  const heading = financial
    ? { eyebrow: s.eyebrow, title: s.title, intro: s.intro }
    : { eyebrow: s.eyebrow, title: t(language, "Record the incident details.", "घटना का विवरण दर्ज करें।"), intro: t(language, "Focus on when it happened, how contact occurred, and what happened. Financial fields are shown only for financial fraud.", "घटना कब हुई, संपर्क कैसे हुआ और क्या हुआ—इन पर ध्यान दें। वित्तीय फ़ील्ड केवल वित्तीय धोखाधड़ी में दिखते हैं।") };

  return <form onSubmit={submit} noValidate><Heading section={heading} /><div className="mt-7 grid gap-5 sm:grid-cols-2">
    ...
    {financial && <>
      <label className="grid gap-2 text-sm font-semibold"><span>{s.amount}</span><input className={fieldClass} type="number" value={report.incident.amount} onChange={(event) => set("amount", event.target.value)} /></label>
    ...
    <label className="grid gap-2 text-sm font-semibold sm:col-span-2"><span>{s.narrative}</span><textarea className="min-h-36 rounded-control border border-line p-3 font-normal" maxLength={600} value={report.incident.narrative} onChange={(event) => set("narrative", event.target.value)} /><span className="font-normal text-muted">{financial ? s.narrativeHelp : t(language, "Describe what happened and what you observed in 40–600 characters.", "40–600 अक्षरों में बताएं कि क्या हुआ और आपने क्या देखा।")} · {report.incident.narrative.length}/600</span></label>
...
function TimelineStep({ complete, fail }: { complete: () => void; fail: (errors: Record<string, string>) => void }) {
  ...
  return <><Heading section={{ eyebrow: s.eyebrow, title: s.title, intro: s.intro }} /><ol className="mt-7 divide-y divide-line border-y border-line">{report.events.map((event, index) => {
...
function ReviewStep({ fail }: { fail: (errors: Record<string, string>) => void }) {
  ...
  const relevantEvidence = evidenceForReportKind(report.evidence, report.reportKind);
  const pathLabel = report.reportKind === "financial" ? t(language, "Financial fraud", "वित्तीय धोखाधड़ी") : report.reportKind === "women-child" ? (report.reportingMode === "anonymous" ? t(language, "Women/Children · eligible anonymous route", "महिला/बाल · पात्र अनाम मार्ग") : t(language, "Women/Children · with details", "महिला/बाल · विवरण के साथ")) : report.reportKind === "other" ? t(language, "Other cybercrime", "अन्य साइबर अपराध") : t(language, "Category not yet clear", "श्रेणी अभी स्पष्ट नहीं") ;
