...
export function MediaLibrary() {
  ...
  const items = MEDIA_ITEMS.filter((item) => (filter === "all" || item.kind === filter) && `${item.title[language]} ${item.summary[language]} ${item.tags.join(" ")}`.toLocaleLowerCase().includes(deferredQuery));
  ...
  return <>
    ...
    {featured ? <>
      <article className="mt-6 grid overflow-hidden rounded-panel border border-line bg-civic-50 md:grid-cols-[0.9fr_1.1fr] md:items-center">
        ...
        <div className="p-6 md:p-8"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-civic-700">{language === "hi" ? "विशेष संसाधन" : "Featured resource"}</p><h2 className="mt-3 text-2xl font-semibold tracking-[-0.02em]">{featured.title[language]}</h2><p className="mt-3 text-sm leading-6 text-muted">{featured.summary[language]}</p><a href={featured.destination} target="_blank" rel="noreferrer" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-control bg-civic-600 px-5 text-sm font-semibold text-white">{language === "hi" ? "आधिकारिक संसाधन खोलें" : "Open official resource"}<ExternalLink className="size-4" /></a></div>
      </article>
      <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{items.slice(1).map((item) => <article key={item.id} className="overflow-hidden rounded-panel border border-line bg-white"><div className="aspect-video overflow-hidden bg-canvas"><ResponsiveIllustration assetId={item.assetId as AssetId} language={language} className="h-full w-full object-cover" /></div><div className="p-5"><p className="text-xs font-medium uppercase tracking-[0.1em] text-civic-700">{item.kind}</p><h2 className="mt-2 text-lg font-medium">{item.title[language]}</h2><p className="mt-2 text-sm leading-6 text-muted">{item.summary[language]}</p><a href={item.destination} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-11 items-center gap-2 text-sm font-medium text-civic-700">{language === "hi" ? "स्रोत खोलें" : "Open source"}<ArrowRight className="size-4" /></a></div></article>)}</div>
