const route = (workspace, mode, archetype, actionModel, heroAsset = "", heroStyle = "standard") => Object.freeze({
  workspace,
  mode,
  archetype,
  actionModel,
...
export const ROUTE_PRESENTATION = Object.freeze({
...
export function workspaceFor(routeId) {
  if (routeId === "home") return "home";
  if (routeId === "track") return "track";
  if (FLOW_ROUTES.has(routeId)) return "report";
  return ROUTE_PRESENTATION[routeId]?.workspace || "home";
}

export function presentationFor(routeId, fallback = "practical-guidance") {
  return ROUTE_PRESENTATION[routeId] || route(workspaceFor(routeId), routeId, fallback, "read");
}
