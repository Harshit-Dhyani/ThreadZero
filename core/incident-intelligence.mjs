...
function paymentEventTitle(amount, paymentMethod, locale) {
...
function buildEvents(text, amount, paymentMethod, happenedAt, locale) {
  const hi = locale === "hi";
  const events = [];
  const add = (id, type, title) => events.push({ id, type, title, occurredAt: happenedAt || null, description: title, evidenceIds: [] });
...
export function buildEvidenceReadiness(incident, locale = "en") {
...
export function refreshIncidentDerivedFields(incident, locale = "en") {
  ...
  ].filter(([, value]) => value).map(([key, value]) => ({ key, value, status: "suggested" }));
  incident.questions = selectQuestions(incident, locale);
  incident.nextActions = selectNextActions(incident, locale);
  return incident;
}

export function selectQuestions(incident, locale = "en") {
  const hi = locale === "hi";
  const questions = [];
...
export function selectNextActions(incident, locale = "en") {
  const hi = locale === "hi";
  const actions = [];
...
export function extractIncident(narrative, locale = "en") {
...
export function createPreparationPack(incident, locale = "en") {
  const hi = locale === "hi";
  const sections = [
